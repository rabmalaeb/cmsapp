import { categoryAdapter, initialState, State } from './state';
...
export function categoryReducer(state = initialState, action: Actions): State {
  switch (action.type) {
    ...
    case ActionTypes.LOAD_SUCCESS: {
      return categoryAdapter.addMany(action.payload.items, {
        ...state,
        isLoading: false,
    ...
    case ActionTypes.DELETE_CATEGORY_SUCCESS: {
      return categoryAdapter.removeOne(action.payload.id, {
        ...state,
        isLoadingAction: false,
        actionError: null
      });
